...
class RLOpponent(Player):
    def __init__(self, model_path, battle_format, account_configuration=None, server_configuration=None):
        ...
        self.model = RecurrentMaskablePPO.load(model_path) if os.path.exists(model_path) else None
    ...
    def choose_move(self, battle):
        ...
        self.episode_starts[0] = False  # Next step won't be episode start
    ...
    def _battle_finished_callback(self, battle):
        # Reset episode start when battle is finished
        self.episode_starts = np.ones((1,), dtype=bool)
        self.lstm_states = None
...
modeldir = "models/recurrent_ppo"
logdir = "logs"
MODEL_PATH = os.path.join(modeldir, f"simpleV5.zip")
NUM_ENVS = 8
...
def train():
    ...
    if os.path.exists(MODEL_PATH):
        model = RecurrentMaskablePPO.load(MODEL_PATH, env=vec_env)
    else:
        model = RecurrentMaskablePPO("MlpLstmPolicy", vec_env, verbose=1,
    ...
    for epoch in range(num_epochs):
        model.learn(
            ...
            tb_log_name="simpleV5_recurrent_ppo",
            progress_bar=True
        )
